...
const NAV_KEYS = [
  ...
  { to: '/teacher/students',  labelKey: 'students',  icon: '◉' },
  { to: '/teacher/reviews',   labelKey: 'reviews',   icon: '◎' },
]
...
export default function TeacherLayout() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-950 flex flex-col">
      <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 focus:z-50 focus:px-3 focus:py-2 focus:bg-emerald-600 focus:text-white focus:rounded-lg text-sm font-medium">
      ...
      <header className="h-14 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-30 flex items-center px-4" role="banner">
        <div className="flex items-center gap-3 flex-1">
          ...
          <span className="hidden sm:inline-flex items-center px-2 py-0.5 rounded-md bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 text-xs font-medium">Teacher</span>
        </div>
      ...
      <div className="flex flex-1 min-h-0">
        <aside className="hidden md:flex flex-col w-52 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 sticky top-14 h-[calc(100vh-3.5rem)]" role="navigation" aria-label="Teacher navigation">
          <nav className="flex-1 py-4 px-3 space-y-0.5">
            {NAV_KEYS.map(item => (
              <NavLink key={item.to} to={item.to}
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-emerald-500 ${
                    isActive
                ...
                <nav className="flex-1 py-3 px-3 space-y-0.5">
                  {NAV_KEYS.map(item => (
                    <NavLink key={item.to} to={item.to} onClick={() => setMobileOpen(false)}
                      className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}>
                      <span className="text-[11px] w-4 text-center opacity-60" aria-hidden="true">{item.icon}</span>
                      {t(item.labelKey)}
                    </NavLink>
                  ))}
