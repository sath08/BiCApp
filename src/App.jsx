...
function RequireStudent({ children }) {
  ...
  if (loading) return <div className="flex items-center justify-center h-screen"><div className="text-purple-600 text-xl animate-pulse">📚 Loading...</div></div>
...
function RequireTeacher({ children }) {
  ...
  if (loading) return <div className="flex items-center justify-center h-screen"><div className="text-teal-600 text-xl animate-pulse">📚 Loading...</div></div>
  if (!user || (userRole !== 'teacher' && userRole !== 'coordinator')) return <Navigate to="/login" replace />
  return children
}

function RequireAdmin({ children }) {
  const { user, userRole, loading } = useAuth()
  if (loading) return <div className="flex items-center justify-center h-screen"><div className="text-orange-600 text-xl animate-pulse">📚 Loading...</div></div>
  if (!user || userRole !== 'coordinator') return <Navigate to="/login" replace />
  return children
}
