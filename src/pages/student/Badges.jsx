...
export default function Badges() {
  const earnedTypes = new Set(mockBadges.map(b => b.badge_type))
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-6">
      <div className="bg-gradient-to-r from-yellow-500 to-orange-500 rounded-2xl p-5 text-white">
        ...
        <p className="text-yellow-100 text-sm">{mockBadges.length} of {BADGE_DEFINITIONS.length} badges earned</p>
      </div>
      ...
      <div className="bg-white rounded-2xl p-4 border border-purple-50 shadow-sm">
        <div className="flex justify-between mb-2">
          ...
          <span className="text-sm font-bold text-yellow-600">{mockBadges.length}/{BADGE_DEFINITIONS.length}</span>
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {BADGE_DEFINITIONS.filter(b => earnedTypes.has(b.type)).map((badge, i) => {
            const earned = mockBadges.find(b2 => b2.badge_type === badge.type)
            return (
              <motion.div
                key={badge.type}
