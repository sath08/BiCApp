    ...
    <div className="space-y-6 pb-20 md:pb-6">
      ...
      <div className="bg-white rounded-2xl p-4 border border-purple-50 shadow-sm">
        <div className="flex justify-between mb-2">
          ...
          <span className="text-sm font-bold text-yellow-800">{badges.length}/{BADGE_DEFINITIONS.length}</span>
        </div>
      ...
      <div>
        ...
        {BADGE_DEFINITIONS.filter(b => earnedTypes.has(b.type)).length === 0 && (
          <p className="text-sm text-gray-600 text-center py-4">No badges yet — keep reading and writing! 🚀</p>
        )}
          ...
          {BADGE_DEFINITIONS.filter(b => earnedTypes.has(b.type)).map((badge, i) => {
            ...
            return (
              ...
              >
                ...
                {earned && <p className="text-xs text-yellow-800 font-semibold mt-2">{new Date(earned.earned_at).toLocaleDateString()}</p>}
                <div className="absolute top-2 right-2">
                  <span className="text-xs bg-yellow-100 text-yellow-800 font-bold px-1.5 py-0.5 rounded-full">✓</span>
                </div>
              </motion.div>
            )
          })}
      ...
      <div>
        <h2 className="font-bold text-gray-600 mb-3">🔒 Locked Badges</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {BADGE_DEFINITIONS.filter(b => !earnedTypes.has(b.type)).map((badge, i) => (
