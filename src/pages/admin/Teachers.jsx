    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-orange-600 to-orange-400 rounded-2xl p-5 text-white flex items-center justify-between">
        <div>
          ...
          <p className="text-orange-100 text-sm">{mockTeachers.length} teachers</p>
        </div>
          ...
          <Card key={t.id} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2 border-t border-gray-100">
              <div className="text-center">
                <p className="font-extrabold text-purple-700 text-xl">{t.student_count}</p>
                <p className="text-xs text-gray-400">Students</p>
              </div>
              <div className="text-center">
                <p className={`font-extrabold text-xl ${t.pending_reviews > 3 ? 'text-red-500' : 'text-green-500'}`}>{t.pending_reviews}</p>
                <p className="text-xs text-gray-400">Pending</p>
              </div>
            </div>
            <div className="flex gap-2">
              <button className="flex-1 text-xs border border-gray-200 rounded-xl py-2 font-semibold text-gray-600 hover:border-teal-300 hover:text-teal-700 transition-colors">Edit</button>
              <button className="flex-1 text-xs border border-gray-200 rounded-xl py-2 font-semibold text-gray-600 hover:border-red-300 hover:text-red-500 transition-colors">Deactivate</button>
            </div>
          </Card>
        ))}
      </div>
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Add New Teacher">
        <form onSubmit={handleSubmit(handleAdd)} className="space-y-4">
          ...
          <Button type="submit" variant="primary" fullWidth>Add Teacher</Button>
        </form>
      </Modal>
    </div>
  )
