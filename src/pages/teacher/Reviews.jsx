...
export default function Reviews() {
  ...
  if (isLoading) return <div className="text-center py-16 text-gray-600">Loading...</div>
            ...
            <Card hover>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600">{r.essay_type} • {r.assignment_type}</p>
                  <p className="text-xs text-gray-600 mt-1">Submitted {new Date(r.submitted_at).toLocaleDateString()}</p>
                  <p className="text-sm text-gray-600 mt-2 line-clamp-2 italic">
                    "{r.essay_text?.substring(0, 120)}..."
                  </p>
                </div>
                <Link to={`/teacher/reviews/${r.id}`}
                  className="bg-gradient-to-r from-teal-700 to-teal-600 text-white font-bold px-5 py-2.5 rounded-xl hover:from-teal-800 hover:to-teal-700 transition-all text-sm whitespace-nowrap shadow-md shadow-teal-200"
                >
                  ✍️ Start Review →
                </Link>
              </div>
