import { motion } from 'framer-motion'
import { Link } from 'react-router-dom'
import { PieChart, Pie, Cell, Tooltip, ResponsiveContainer } from 'recharts'
import StatCard from '../../components/ui/StatCard'
import Card from '../../components/ui/Card'
import { mockStudents, mockPendingReviews } from '../../lib/mockData'

const pieData = [
  { name: 'Approved', value: 12, color: '#10B981' },
  { name: 'Under Review', value: 3, color: '#3B82F6' },
  { name: 'Pending', value: 5, color: '#F59E0B' },
]

export default function TeacherDashboard() {
  const myStudents = mockStudents.slice(0, 5)

  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-teal-700 to-teal-500 rounded-2xl p-6 text-white">
        <p className="text-teal-200 font-medium mb-1">Welcome back,</p>
        <h1 className="text-2xl font-extrabold">Ms. Rodriguez 👩‍🏫</h1>
        <p className="text-teal-200 text-sm mt-1">Bellevue Reading & Writing Champions</p>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard emoji="👥" label="My Students" value={12} color="teal" index={0} />
        <StatCard emoji="📝" label="Pending Reviews" value={3} color="orange" index={1} />
        <StatCard emoji="✅" label="Completed" value={18} color="green" index={2} />
        <StatCard emoji="⏱️" label="Avg Review Time" value="2.3 days" color="purple" index={3} />
      </div>

      {/* Pending Reviews Alert */}
      {mockPendingReviews.length > 0 && (
        <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-bold text-orange-800 flex items-center gap-2">
              <span>⚠️</span> Essays Awaiting Review
            </h2>
            <Link to="/teacher/reviews" className="text-xs font-bold text-orange-700 bg-orange-100 px-3 py-1.5 rounded-lg hover:bg-orange-200">
              View All →
            </Link>
          </div>
          <div className="space-y-2">
            {mockPendingReviews.map(r => (
              <div key={r.id} className="flex items-center justify-between bg-white rounded-xl p-3">
                <div>
                  <p className="font-semibold text-sm text-gray-800">{r.student_anonymous_id}</p>
                  <p className="text-xs text-gray-500">{r.essay_type} — {r.book_title}</p>
                </div>
                <Link to={`/teacher/reviews/${r.id}`} className="text-xs font-bold text-teal-700 bg-teal-50 px-3 py-1.5 rounded-lg hover:bg-teal-100">
                  Review →
                </Link>
              </div>
            ))}
          </div>
        </div>
      )}

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {/* Review Status Chart */}
        <Card>
          <h3 className="font-bold text-purple-900 mb-4">📊 Review Status</h3>
          <ResponsiveContainer width="100%" height={180}>
            <PieChart>
              <Pie data={pieData} cx="50%" cy="50%" innerRadius={50} outerRadius={80} dataKey="value" paddingAngle={3}>
                {pieData.map((entry, i) => <Cell key={i} fill={entry.color} />)}
              </Pie>
              <Tooltip />
            </PieChart>
          </ResponsiveContainer>
          <div className="flex justify-center gap-4 mt-2">
            {pieData.map(p => (
              <div key={p.name} className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded-full" style={{ background: p.color }} />
                <span className="text-xs text-gray-600">{p.name} ({p.value})</span>
              </div>
            ))}
          </div>
        </Card>

        {/* My Students */}
        <Card>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-purple-900">👥 My Students</h3>
            <Link to="/teacher/students" className="text-xs text-teal-600 font-semibold hover:text-teal-800">View all →</Link>
          </div>
          <div className="space-y-2">
            {myStudents.map(s => (
              <div key={s.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div>
                  <p className="font-semibold text-sm text-gray-800">{s.first_name} {s.last_name}</p>
                  <p className="text-xs text-gray-400">Grade {s.grade} • {s.anonymous_id}</p>
                </div>
                <span className="font-bold text-purple-700 text-sm">{s.total_points} XP</span>
              </div>
            ))}
          </div>
        </Card>
      </div>
    </div>
  )
}
